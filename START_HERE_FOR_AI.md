...
- Active next slice: `CC-PHASE-2.1-BASELINE-DEFINITION-LOCKS-AND-NEXT-LEGITIMATE-FILLS-1.0` (Phase 2 remains PARTIAL; expansion continuation shipped)
...
- **Equal Standing Before the Constitution** is a foundational constitutional principle (`CC-PRIN-29`, `CC-DEV-062`–`067`) — equal protection and equal accountability; Blind Stage Principle (conduct before identity); Equality of Opportunity and Accountability; reject privilege in every direction; equal access; shared civic identity; Equal Standing Test (`CC-DEC-065`/`066`); iconic guide: *The Constitution should hear the evidence before it sees the person*; distinguishes equal standing from identical treatment and from equality of outcome; complements Declaration *On equality*; do not invent access-to-justice statistics
- **Constitutional Citizenship Doctrine** is a core doctrine (`CC-PRIN-30`/`31`, `CC-DEV-068`–`077`) — self-government requires education; democracy as a skill; preparation over voting-age alone; Youth Leadership Councils (adults remain legally responsible); Civic Graduation; schools as community laboratories; **viewpoint-neutral civic service** (participation, not partisan conformity; optional campaign credit only under equal standards; never required for graduation); Civic Service Portfolio + Civic Achievement Transcript; government should encourage participation, not ideology; earlier local voting only as pilots — do **not** declare voting begins at 16; no invented civics-outcome statistics (`CC-DEC-067`/`068`/`069`)
- **Constitutional Family Doctrine** is a major pillar (`CC-PRIN-32`, `CC-DEV-078`–`084`) — children are the greatest long-term asset; family-first care ladder; reject “productive” as a worth test; no child should go hungry (local first, state last resort); Family Wealth System; child flourishing measurement (`CC-DEC-070`/`071`); no invented childhood statistics
- **Human Flourishing Index** (`CC-PRIN-33`, `CC-DEV-088`) is the defining benchmark question — *Did human life improve?* / *Does this measurably increase human flourishing?* — design agenda only; 0 domains sourced; no invented composites (`CC-DEC-072`)
- **Living philosophy organizing artifacts:** `data/project/living_project_identity.json`, `data/project/systems_map.json`, `data/research/knowledge_graph.json` (Burt expands graph in 2.1) — architecture ≠ proof; Phase 2 remains PARTIAL; modeling/legal **0%**
